...
function mainLoop() {
	...
	sites.forEach( function(site) {
        fetches.push( fetchURL( listConfig.client[site].url, listConfig.client[site].clientName ) ); 
    });
    Promise.each( fetches, function( fetchResult ) {
        if ( fetchResult.fetchedTitle ) {
            ...
            badMsg += fetchResult.name + " is NOT up.<br/>";
        }
  	}).then( function(){
...
// ===============================
function fetchURL (url, clientName) {
